...
def main():
    ...
    # 加载Optuna最佳参数
    with open(PROCESSED_DIR + 'best_params.json', 'r') as f:
        best_params = json.load(f)

    print(f"\nUsing Optuna optimized parameters:")
    print(f"Best Log Loss from Optuna: 0.5179")
    print(f"Parameters: {best_params}")

    # 使用Optuna优化后的参数定义LightGBM
    ...
    ensemble = VotingClassifier(
        ...
        voting='soft',
        weights=[0.35, 0.25, 0.25, 0.15]  # 给Optuna优化后的LGB更高权重
    )
    ...
    print("\nTraining Final Ensemble with Optuna optimized LightGBM...")
    ...
    submission.to_csv(PROCESSED_DIR + 'submission_final_optuna.csv', index=False)
    print("\nFinal submission with Optuna saved!")
